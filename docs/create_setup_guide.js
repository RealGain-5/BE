...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({
        alignment: AlignmentType.CENTER,
        children: [new TextRun({ text: "Git 기반 프로젝트 설치 · 환경 구성 완전 가이드", size: 24, bold: true, color: C.h2Blue })]
      }),
      sp(),
      new Paragraph({
        alignment: AlignmentType.CENTER,
        children: [new TextRun({ text: "대상 독자: Git 및 개발 환경 초보자  |  2026년 5월", size: 20, color: C.gray })]
      }),
      ...
      h(HeadingLevel.HEADING_2, "1.1 Git이란?"),
      p("Git은 소스 코드를 관리하고 공유하는 도구입니다. 마치 문서 작업 시 '버전 관리(v1, v2, 최종, 진짜최종…)'를 체계적으로 해 주는 프로그램이라고 생각하면 됩니다."),
      p("GitHub(https://github.com)는 Git으로 관리되는 코드를 인터넷에 올려두는 저장소 서비스입니다. 이 프로젝트 코드도 GitHub에 저장되어 있으며, Git 명령어로 내 컴퓨터에 내려받을 수 있습니다."),
      sp(),
      ...
      p("아래 4가지 프로그램을 순서대로 설치합니다. 이미 설치되어 있는 항목은 건너뛰어도 됩니다."),
      sp(),
      ...
      h(HeadingLevel.HEADING_2, "2.1 Git 설치"),
      p("① 웹 브라우저에서 https://git-scm.com 에 접속합니다."),
      p("② 화면 중앙의 [Download for Windows] 버튼을 클릭해 설치 파일을 내려받습니다."),
      p("③ 내려받은 파일을 실행하고, 모든 설치 옵션을 기본값(Next)으로 진행하여 설치를 완료합니다."),
      p("④ PowerShell을 새로 열고 아래 명령어를 입력해 설치를 확인합니다."),
      code("git --version"),
      p("결과 예시: git version 2.47.0.windows.2"),
      sp(),

      h(HeadingLevel.HEADING_2, "2.2 Node.js 설치"),
      ...
      h(HeadingLevel.HEADING_2, "2.3 Python 설치"),
      ...
      // ── 3. 코드 내려받기 ──────────────────────────────────
      new Paragraph({ children: [new PageBreak()] }),
      h(HeadingLevel.HEADING_1, "3. STEP 2 — Git으로 코드 내려받기 (클론)"),

      h(HeadingLevel.HEADING_2, "3.1 프로젝트를 저장할 폴더 위치 결정"),
      p("코드를 저장할 폴더를 먼저 정합니다. 아래 예시에서는 D:\\projects 폴더를 사용합니다. 원하는 경로로 변경해도 됩니다."),
      ...callout("예시 경로", [
      ...
      h(HeadingLevel.HEADING_2, "3.2 폴더 이동 및 클론 실행"),
      ...
      sp(),
      p("② GitHub에서 코드를 내 컴퓨터로 내려받습니다. (클론)"),
      code("git clone https://github.com/RealGain-5/BE.git rcp_5th"),
      p("Cloning into 'rcp_5th'... 메시지와 함께 파일이 내려받아집니다. 네트워크 속도에 따라 30초~2분 소요됩니다."),
      sp(),
      p("③ 프로젝트 폴더 안으로 이동합니다."),
      code("cd rcp_5th"),
      sp(),
      p("④ 파일이 제대로 내려받아졌는지 확인합니다."),
      code("ls"),
      ...
      p("아래 파일들은 Git 저장소에 포함되지 않으므로 담당자에게 별도로 요청해야 합니다."),
      sp(),
      ...
      h(HeadingLevel.HEADING_2, "7.3 .gitignore 제외 항목 전체 목록"),
      p("아래는 Git 저장소에서 의도적으로 제외된 항목입니다."),
      sp(),
      makeTable(
        ["제외 항목", "이유", "조치"],
        [
      ...
      makeTable(
        ...
        [
          [
            ...
            "Git이 설치되지 않았거나 PATH 미등록",
            "Git을 재설치하고 PowerShell을 새로 열어 재시도"
          ],
          [
      ...
      num("Git 설치 → git --version 확인", "num-1"),
      num("Node.js 설치 → node --version 확인", "num-1"),
      num("Python 설치 (Add to PATH 필수) → python --version 확인", "num-1"),
      num("git clone https://github.com/RealGain-5/BE.git rcp_5th", "num-1"),
      num("cd rcp_5th", "num-1"),
